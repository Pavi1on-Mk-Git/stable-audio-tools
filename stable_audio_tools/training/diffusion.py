...
from ..interface.aeiou import audio_spectrogram_image, pca_point_cloud, tokens_spectrogram_image
from ..models.autoencoders import DiffusionAutoencoder
from ..models.diffusion import ConditionedDiffusionModelWrapper, DiffusionModelWrapper
...
class DiffusionUncondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(self, model: DiffusionModelWrapper, lr: float = 1e-4, pre_encoded: bool = False):
        super().__init__()

        self.diffusion = model

        self.diffusion_ema = EMA(self.diffusion.model, beta=0.9999, power=3 / 4, update_every=1, update_after_step=1)
...
class DiffusionUncondDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            log_audio(trainer.logger, "demo", filename, sample_rate=self.sample_rate, caption="Reconstructed")
            log_image(trainer.logger, "demo_melspec_left", audio_spectrogram_image(fakes))
...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        self.loss_modules = [
            MSELoss(
                "output", "targets", weight=1.0, mask_key="padding_mask" if self.mask_padding else None, name="mse_loss"
            )
        ]
        ...
        assert lr is not None or optimizer_configs is not None, (
            "Must specify either lr or optimizer_configs in training config"
        )

        if optimizer_configs is None:
            optimizer_configs = {"diffusion": {"optimizer": {"type": "Adam", "config": {"lr": lr}}}}
        else:
            if lr is not None:
                print(
                    "WARNING: learning_rate and optimizer_configs both specified in config. Ignoring learning_rate and using optimizer_configs."
                )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if isinstance(first_padding_mask, list) and len(first_padding_mask) == 1:
            padding_masks = torch.stack([md["padding_mask"][0] for md in metadata], dim=0).to(
                self.device
            )  # Shape (batch_size, sequence_length)
        else:
            padding_masks = torch.stack([md["padding_mask"] for md in metadata], dim=0).to(
                self.device
            )  # Shape (batch_size, sequence_length)
        ...
        if self.diffusion.pretransform is not None:
            ...
            if not self.pre_encoded:
                with torch.cuda.amp.autocast() and torch.set_grad_enabled(self.diffusion.pretransform.enable_grad):
                    ...
                    padding_masks = (
                        F.interpolate(padding_masks.unsqueeze(1).float(), size=diffusion_input.shape[2], mode="nearest")
                        .squeeze(1)
                        .bool()
                    )
            else:
        ...
        if self.timestep_sampler == "uniform":
            ...
            t = sample_timesteps_logsnr(reals.shape[0], mean_logsnr=self.mean_logsnr, std_logsnr=self.std_logsnr).to(
                self.device
            )
        else:
        ...
        output = self.diffusion(
            noised_inputs, t, cond=conditioning, cfg_dropout_prob=self.cfg_dropout_prob, **extra_args
        )
        p.tick("diffusion")
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        try:
            ...
            if self.display_audio_cond:
                ...
                log_audio(trainer.logger, "demo_audio_cond", filename, self.sample_rate)
                log_image(trainer.logger, "demo_audio_cond_melspec_left", audio_spectrogram_image(audio_inputs))
            ...
            if self.cond_display_configs is not None:
                for cond_display_config in self.cond_display_configs:
                    ...
                    if cond_type == "audio":
                        ...
                        log_image(
                            trainer.logger, f"demo_{cond_id}_melspec_left", audio_spectrogram_image(audio_inputs_out)
                        )
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                with torch.cuda.amp.autocast():
                    model = (
                        module.diffusion_ema.ema_model if module.diffusion_ema is not None else module.diffusion.model
                    )
                    ...
                    for cond_display_config in self.cond_display_configs:
                        ...
                        if cond_type == "audio":
                            ...
                            if display_mix:
                                ...
                                log_audio(
                                    trainer.logger, f"demo_{cond_id}_mix_cfg_{cfg_scale}", filename, self.sample_rate
                                )
                            ...
                            log_audio(
                                trainer.logger, f"demo_{cond_id}_submix_cfg_{cfg_scale}", filename, self.sample_rate
                            )
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        try:
            ...
            if not module.pre_encoded:
                ...
                log_image(
                    trainer.logger,
                    "demo_reals_melspec_left",
                    audio_spectrogram_image(rearrange(demo_reals, "b d n -> d (b n)").mul(32767).to(torch.int16).cpu()),
                )
            ...
            if module.diffusion.pretransform is not None:
                log_image(trainer.logger, "demo_masked_input", tokens_spectrogram_image(masked_input.cpu()))
            else:
                log_image(
                    trainer.logger,
                    "demo_masked_input",
...
class DiffusionAutoencoderTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        self, model: DiffusionAutoencoder, lr: float = 1e-4, ema_copy=None, use_reconstruction_loss: bool = False
    ):
        super().__init__()
        ...
        if use_reconstruction_loss:
            ...
            loss_modules.append(
                AuralossLoss(
                    self.sdstft, "audio_reals", "audio_pred", name="mrstft_loss", weight=0.1
                ),  # Reconstruction loss
            )
...
class DiffusionAutoencoderDemoCallback(pl.Callback):
    def __init__(self, demo_dl, demo_every=2000, demo_steps=250, sample_size=65536, sample_rate=48000):
        super().__init__()
        self.demo_every = demo_every
        self.demo_steps = demo_steps
        self.demo_samples = sample_size
    ...
    def on_train_batch_end(self, trainer, module: DiffusionAutoencoderTrainingWrapper, outputs, batch, batch_idx):
        ...
        reals_fakes = (
            reals_fakes.to(torch.float32).div(torch.max(torch.abs(reals_fakes))).mul(32767).to(torch.int16).cpu()
        )
        ...
        log_audio(trainer.logger, "recon", filename, sample_rate=self.sample_rate, caption="Reconstructed")
        ...
        if module.diffae_ema.ema_model.pretransform is not None:
            with torch.no_grad() and torch.cuda.amp.autocast():
                ...
                log_image(trainer.logger, "first_stage_latents", tokens_spectrogram_image(initial_latents))
                log_image(trainer.logger, "first_stage_melspec_left", audio_spectrogram_image(first_stage_fakes))
